...
/**
 * Task 7 (+ cutover Task 6): command `marketplace:push-stock` — pembungkus
 * tipis di atas MarketplaceMasterService::pushDirty(), dipanggil scheduler
 * tiap 5 menit (routes/console.php). Command sendiri tak menyentuh DB/HTTP —
 * itu semua tanggung jawab service (sudah diuji di tests/Feature/MarketplaceMaster).
 * Di sini cukup pastikan command memanggil pushDirty() sekali dan melaporkan
 * hasilnya.
 */
class PushCommandTest extends TestCase
{
    ...
    {
        $this->mock(MarketplaceMasterService::class, function ($m) {
    ...
    public function test_command_melaporkan_ringkasan_angka_pada_output(): void
    {
        ...
        $this->artisan('marketplace:push-stock')
            ->assertExitCode(0)
            ->expectsOutputToContain('Push stok+harga: 3 terkirim · 1 dilewati · 2 gagal.');
    }
}
