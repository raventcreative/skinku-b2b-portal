    ...
    @if(count($unmapped))
        <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs whitespace-nowrap">
                    <thead class="bg-stone-50 text-stone-500 uppercase text-[10px]"><tr>
                        <th class="text-left px-4 py-2">Seller SKU</th><th class="text-left">Judul</th><th class="text-left pr-4">Tautkan ke Produk</th>
                    </tr></thead>
                    <tbody>
                        @foreach($unmapped as $l)
                            <tr class="border-t border-stone-100">
                                ...
                                <td class="py-2 pr-4">
                                    <form method="POST" action="{{ route('marketplace-stock.tautkan', ['channel' => $channel]) }}" class="flex items-center gap-1.5">
                                        @csrf
