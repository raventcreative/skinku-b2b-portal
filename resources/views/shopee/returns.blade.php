@extends('layouts.app')
@section('title', 'Retur Shopee')
@section('heading', 'Retur Shopee — Review Layak Jual')

@section('content')
<a href="{{ route('shopee.index') }}" class="text-xs text-stone-500 hover:text-stone-800">← Kembali ke Integrasi</a>

<div class="mt-3 flex flex-wrap items-center gap-3">
    <form method="POST" action="{{ route('shopee.returns.sync') }}">@csrf
        <button class="px-4 py-2 text-sm bg-stone-800 text-white rounded-lg hover:bg-stone-900">↻ Tarik Retur dari Shopee</button>
    </form>
...
<div class="mt-4 space-y-2">
    @forelse($returns as $r)
        ...
        <div class="bg-white rounded-2xl border border-stone-200 p-4 flex flex-col sm:flex-row sm:items-start gap-3">
            <div class="sm:w-56 sm:shrink-0">
                <div class="font-mono text-xs text-stone-700 break-all">{{ $r->shopee_return_sn }}</div>
                <div class="text-[10px] text-stone-400">Order {{ $r->shopee_order_sn ?? '—' }} · {{ $r->return_created_at?->format('d M Y') ?? '—' }}</div>
                <span class="inline-block mt-1 px-2 py-0.5 rounded-full bg-stone-100 text-stone-500 text-[10px] font-bold">{{ $r->status ?? 'retur' }}</span>
            </div>
            ...
            <div class="sm:w-48 sm:shrink-0 sm:text-right">
                @if($r->review_status === \App\Models\ShopeeReturn::REVIEW_RESTOCKED)
                    ...
                    <form method="POST" action="{{ route('shopee.returns.reset', $r) }}" class="inline" onsubmit="return confirm('Batalkan? Stok yang tadi ditambah akan ditarik lagi.')">@csrf
                ...
                @elseif($r->review_status === \App\Models\ShopeeReturn::REVIEW_REJECTED)
                    ...
                    <form method="POST" action="{{ route('shopee.returns.reset', $r) }}" class="inline">@csrf
                ...
                @else
                    <div class="flex sm:flex-col gap-1.5 sm:items-end">
                        @if($pv['all_matched'])
                            <form method="POST" action="{{ route('shopee.returns.restock', $r) }}" onsubmit="return confirm('Barang layak jual — tambah stok?')">@csrf
                        ...
                        @else
                            <span class="text-[10px] text-rose-500">petakan SKU dulu (di SKU &amp; Stok)</span>
                        @endif
                        <form method="POST" action="{{ route('shopee.returns.reject', $r) }}" onsubmit="return confirm('Tandai cacat / tidak layak jual? Stok tidak ditambah.')">@csrf
    ...
    @empty
        <div class="bg-white rounded-2xl border border-stone-200 px-4 py-8 text-center text-stone-400 text-sm">Belum ada retur. Klik "Tarik Retur dari Shopee".</div>
    @endforelse
</div>
