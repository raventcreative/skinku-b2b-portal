...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    <div class="flex gap-1 bg-stone-100 rounded-xl p-1 text-sm">
        @foreach($tabs as $key => [$label, $url])
            <a href="{{ $url }}?period={{ $period }}" class="px-4 py-1.5 rounded-lg {{ $tab === $key ? 'bg-white shadow-sm font-semibold text-red-700' : 'text-stone-600 hover:text-stone-900' }}">{{ $label }}</a>
        @endforeach
    </div>
