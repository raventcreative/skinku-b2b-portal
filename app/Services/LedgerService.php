...
class LedgerService
{
     ...
     * Saldo bersih (debit − kredit) diletakkan di kolom debit/kredit sesuai tanda.
     * Total debit == total kredit (karena Σ posted debit == Σ posted credit).
     *
    ...
    public function trialBalance(?string $asOfPeriod = null, ?int $branchId = null): array
    {
        $agg = AccJournalLine::query()
            ...
            ->when($asOfPeriod, fn ($q) => $q->where('acc_journals.period', '<=', $asOfPeriod))
            ->groupBy('acc_accounts.id', 'acc_accounts.code', 'acc_accounts.name', 'acc_accounts.type', 'acc_accounts.normal_balance')
            ->orderBy('acc_accounts.code')
            ->selectRaw('acc_accounts.id, acc_accounts.code, acc_accounts.name, acc_accounts.type, acc_accounts.normal_balance,
