...
- **10% join → PEREKRUT (dikunci user 2026-08-21):** `CommissionService::recordJoinBonus` diubah — penerima = **`member->sponsor_id`**. Kalau `sponsor_id` null (daftar mandiri / tanpa perekrut) → **TAK ADA join dibayar** (nobody; HQ simpan penuh) — konsisten dgn 5% RO (no perekrut → no reward), BUKAN fallback ke upline. Onboarding meng-set `sponsor_id`; `JoinBonusTest` + tes onboarding existing diupdate (join sekarang ke sponsor, bukan upline). Type tetap `join`. 10% dari **nilai paket** (mis. paket GD 49jt → 4,9jt); admin bikin paket GD via JoinPackage `target_role=grand_distributor`.
