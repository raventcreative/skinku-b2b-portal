...
**Status:** Engine dasar (override per-channel ala Desty) SELESAI & LIVE di `main`. Redesign katalog (dedup by NAMA, tab Satuan/Bundle, foto, gabung, hapus master, aksi gabungan "Siapkan Master") SELESAI di branch `feat/produk-master-katalog` (5 task SDD), belum merge ke `main`.

**Tujuan:** kontrol stok+harga marketplace ala Desty — tiap unit jualan (satuan/varian/**bundle**) = 1 baris "Produk Master" dengan stok & harga **di-set langsung** (bukan diturunkan dari pool produk HQ), dipetakan ke listing TikTok/Shopee, dengan opsi override per-channel. **TERPISAH TOTAL dari stok HQ** (`hq_stock`/`stock_movements` tak pernah dibaca/ditulis oleh modul ini). Rework dari model Fase 1/1.5 ("Kontrol Stok Marketplace" — pool per-`Product` + override per `(product,channel)`, tabel `marketplace_stocks`/`marketplace_channel_overrides`): tabel lama itu **sudah di-drop** (migrasi `000146`, data lama dimigrasikan ke master dulu bila ada).

**Dedup by NAMA (bukan SKU):** dua listing lintas channel/varian yang namanya sama (mis. beda cuma spasi/kapital) otomatis digabung jadi 1 master walau seller_sku beda — kunci dedup = `name_key` (nama ternormalisasi: lower + squish spasi), BUKAN lagi `master_sku`. Constraint unique `master_sku` sudah di-drop (migrasi `000148`) — SKU cuma "perwakilan" (SKU dari listing pertama yang bikin master), boleh sama antar master kalau 2 channel kebetulan pakai SKU sama utk produk beda judul.

### Model & tabel
| Model | Tabel | Kolom kunci |
|---|---|---|
| `MarketplaceMaster` | `marketplace_masters` | `master_sku` (perwakilan, TAK unik lagi), `name`, `name_key` (indeks biasa — dedup ditegakkan di level aplikasi lewat `findOrCreateMaster()`, bukan constraint DB), `is_bundle` (auto-deteksi dari nama saat master dibuat, bisa di-toggle manual), `image_url` (auto dari resolve channel, backfill-only), `product_id` (nullable, referensi opsional saja — **bukan** sumber stok), `base_stock` (nullable = belum di-set → tak di-push), `base_price`, `seeded_at` (patokan reconcile order-mirror) |
...
| `MarketplaceListing` | `marketplace_listings` (dari Fase 1) | + `master_id` (nullable, FK ke master, `nullOnDelete`), + jejak push harga: `last_pushed_price`/`last_price_status`/`last_price_error`/`last_price_pushed_at` (jejak push stok — `last_pushed_qty`/`last_status`/`last_error`/`last_pushed_at` — sudah ada sejak Fase 1) |

Migrasi: `000139`/`000140` (tabel Fase 1 — **di-drop** oleh `000146`), `000145` (tabel `marketplace_masters`/`marketplace_master_channels` + kolom master/harga di `marketplace_listings`), `000146` (migrasi data `marketplace_stocks`→`base_stock` Master + `marketplace_channel_overrides`→`MarketplaceMasterChannel`, guard `Schema::hasTable()` biar aman baik tabel lama ada maupun tidak, lalu `dropIfExists` keduanya), `000147` (kolom `name_key`/`is_bundle`/`image_url` + **reset master lama** — data lama dibentuk per-seller_sku/fragmented, dihapus & `marketplace_listings.master_id` di-null-kan, dibangun ulang by NAMA lewat tombol "Siapkan Master"; data mirror marketplace, rebuildable & aman baik di prod maupun DB kosong/test), `000148` (drop unique `master_sku`, cegah bentrok SKU sama beda-judul lintas channel).

Model helper (`MarketplaceMaster`): `normalizeName($name)` (lower+squish, dipakai bikin `name_key`), `detectBundle($name)` (regex `bundl|paket`, dipakai saat master baru dibuat), `imageUrl()` (foto upload manual — koleksi `master_image` via trait `HasFiles` + `ImageService::attach()`, sistem file polymorphic yang sama dipakai modul lain — **menang** atas `image_url` hasil auto-resolve channel, fallback ke situ kalau belum ada upload).
...
- **Resolve & tautkan** — `resolveListings(channel)` tarik listing terbaru dari TikTok/Shopee + auto-`findOrCreateMaster()` **by NAMA** (`name_key`) untuk listing yang belum bermaster; foto ikut ditarik (`main_images.0.urls.0`/`thumb_urls.0` TikTok, `image.image_url_list.0` Shopee) dan di-backfill ke master yang belum punya foto (tak menimpa upload manual / hasil resolve channel lain). Listing yang **sudah** tertaut (mis. hasil gabung manual lintas SKU) tak disentuh sama sekali — mencegah master orphan baru tercipta tiap resolve ulang. `tautkanListing(listing, masterId?, newSku?, newName?)` — pindahkan listing ke master lain (gabung) atau buat master baru.
- **Siapkan Master** — `siapkanMaster()`: jalankan `resolveListings()` TikTok+Shopee sekaligus (per-channel try/catch, satu channel gagal tak menggagalkan yang lain) lalu `deleteOrphanMasters()`. Ini tombol utama di halaman katalog, menggantikan alur manual "Refresh Listing" per channel utk pemakaian sehari-hari.
- **Gabung master** — `mergeMaster(source, target)`: pindahkan semua listing `source` ke `target`, lalu hapus `source`. No-op aman kalau `source->id === target->id` (guard di service; dropdown UI juga sudah exclude master dirinya sendiri, jadi self-merge tak tercapai dari UI).
- **Hapus master** — `deleteOrphanMasters()` (dipanggil dari `siapkanMaster`) hanya menghapus master yang SAMA SEKALI tak punya listing, dan **skip** (tak hapus) orphan yang sudah dikonfigurasi manual (`base_stock`/`base_price`/foto terisi) — cegah kehilangan data diam-diam. Hapus manual satu master (tombol "Hapus" di katalog) langsung `delete()` — listing terkait jadi `master_id = null` (bisa ditautkan ulang), `marketplace_master_channels` ikut cascade.
...
Tabel ala Desty — kolom **Foto · Produk (nama + badge BUNDLE) · Master SKU · Harga · Stok · Channel Terkait · Atur**. Harga/Stok = form inline (submit langsung `setMasterPrice`/`setMasterStock` + push ke channel). "Channel Terkait" ringkas status kirim terakhir tiap channel (`last_status`/`last_price_status`, lewat `channelSummary()`).

- **Tab** `?tab=semua|satuan|bundle` (default `semua`) — filter kolom `is_bundle`, label tab menampilkan hitungan (`counts`). Baris tabel DAN dropdown "Gabung" (lihat bawah) di-scope tab yang sama, supaya produk kategori lain tak ikut "bocor" ke HTML tab yang sedang aktif.
- **Banner** ringkas listing belum termaster (`unmasteredCount`) — bukan lagi tabel besar terpisah spt versi lama, cuma ajakan klik "Siapkan Master".
- **Atur** (dropdown per baris): Sinkron (`push`), toggle Jadikan Satuan/Bundle (`master.bundle`), upload Foto (`master.foto`, form `enctype="multipart/form-data"`), Gabung ke master lain (`master.gabung` — `<select>` isi semua master **selain dirinya sendiri**), Hapus (`master.hapus`, method `DELETE` + konfirmasi JS `confirm()`).
- Toolbar atas: "Siapkan Master (TikTok+Shopee)" (`siapkan`) + "Sinkron semua" (`push-all`) + link ke halaman override per-channel TikTok/Shopee (`channel`).

### Alur
1. **Siapkan Master** — tombol "Siapkan Master (TikTok+Shopee)" (`/marketplace-stock/siapkan`) → `siapkanMaster()`: resolve TikTok+Shopee sekaligus (isi/segarkan `marketplace_listings` + auto-buat/gabung `marketplace_masters` by nama) lalu bersihkan master orphan. Ini jalur utama sehari-hari; "Refresh Listing" per-channel (`/marketplace-stock/resolve`) tetap ada di halaman override per-channel utk kebutuhan spesifik 1 channel.
2. **Rapikan katalog** — dari tab Satuan/Bundle: toggle bundle, upload foto manual, gabung 2 master yang ternyata produk sama (nama beda tipis/lintas channel), atau hapus master yang tak relevan.
3. **Set Master** — admin isi stok/harga langsung di baris Produk Master (`/marketplace-stock`) → `pushMaster()` sinkron seketika ke semua channel yang listing-nya sudah ter-resolve.
4. **Override per channel** — admin isi stok/harga khusus 1 channel (`/marketplace-stock/{channel}`); "Ikut Master" hapus override, kembali mengikuti nilai Master.
5. **Push berkala** — cron `marketplace:push-stock` tiap 5 menit → `pushDirty()` (hanya listing yang nilainya berubah sejak push terakhir; §20).
6. **Order masuk** — `TikTokOrderService`/`ShopeeOrderService` potong stok HQ (jalur lama, tak berubah) **dan** panggil `applyOrderDelta()` supaya bucket master/override ikut turun — HQ tak disentuh oleh jalur ini.
7. **Seed awal** — tombol "Tarik Stok Awal dari TikTok" → `seedFromTiktok()`.

### Route / izin
`MarketplaceStockController` (nama kelas dipertahankan dari Fase 1 demi kestabilan URL `/marketplace-stock*`; isinya kini 100% engine Produk Master) — rute index/channel/setMasterStock/setMasterPrice/setChannelStock/setChannelPrice/ikutMaster/tautkan/masterize-all/push/pushAll/resolve/seed/siapkan/master.hapus (`DELETE`)/master.bundle/master.gabung/master.foto. View pakai `<select>`/input native, POST + `@csrf` biasa (tanpa literal `@json([...])`); upload foto pakai `enctype="multipart/form-data"`.
