...
Model helper (`MarketplaceMaster`): `normalizeName($name)` (lower+squish, dipakai bikin `name_key`), `detectBundle($name)` (regex `bundl|paket` — HANYA dipakai `findOrCreateMaster()`, bukan form manual yang pakai select Tipe eksplisit), `imageUrl()` (foto upload manual — koleksi `master_image` via trait `HasFiles` + `ImageService::attach()` — **menang** atas `image_url`, fallback ke situ kalau belum ada upload).
...
- **Duplikat** — `duplicateMaster(MarketplaceMaster): MarketplaceMaster` — gandakan `name` ("... (copy)"), `master_sku` ("...-COPY"), `is_bundle`, `base_price` ke master baru; TANPA listing/channel-override/foto/`base_stock` — mulai bersih, controller redirect ke form Ubah hasil duplikat supaya admin sesuaikan lalu simpan.
- **Refresh listing (TANPA auto-master)** — `resolveListings(channel)` HANYA upsert baris `marketplace_listings` (item_id/variation_id/warehouse_id/title) dari TikTok/Shopee; `master_id` listing baru dibiarkan `null` — tak lagi auto-`findOrCreateMaster()` seperti model sebelumnya (`RefreshNoAutoMasterTest`). Penautan ke master sekarang murni manual lewat `tautkanListing()`.
- **Tautkan** — `tautkanListing(listing, masterId?, newSku?, newName?)`: kalau `$masterId` diisi → pindahkan listing ke master itu (jalur utama, dipicu tombol **"Tambah ke Marketplace"** di katalog — admin pilih master lalu pilih 1 listing belum tertaut); kalau `null` → cari-atau-buat master baru by nama (`findOrCreateMaster()`, dedup by `name_key`) lalu tautkan — jalur ini tetap ada & dites (`TautkanTest`) tapi tak (lagi) punya entry point di UI katalog saat ini (dipanggil langsung ke controller `tautkan` dengan `master_id` kosong).
...
Tabel ala Desty — kolom **Foto · Informasi Produk (nama + badge BUNDLE) · Master SKU · Harga · Stok · Produk Terkait · Toko Terkait · Atur**. Harga/Stok = form inline (submit langsung `setMasterPrice`/`setMasterStock`, lalu push otomatis ke channel yang listing-nya sudah tertaut). "Produk Terkait" = jumlah listing tertaut (`listings->count()`); "Toko Terkait" = jumlah channel unik tertaut (`listings->pluck('channel')->unique()->count()`); keduanya "—" bila 0.
...
- **Atur** (dropdown per baris): **Ubah** (`edit`/`update`) · **Duplikat Produk** (`duplikat`) · **Tambah ke Marketplace** (submenu — `<select>` isi listing yang belum tertaut (`unlinkedListings`), POST `tautkan` dgn `master_id` = master baris ini) · **Jadikan Bundle/Satuan** (toggle, `master.bundle`) · **Hapus** (`master.hapus`, method `DELETE` + konfirmasi JS `confirm()`).
...
- Form Tambah/Ubah (`marketplace-stock.form`, satu view utk `create`+`edit`): Nama, Master SKU, Harga & Stok (nullable — boleh kosong dulu, mis. utk bundle), Tipe (`<select>` Satuan/Bundle), Foto (`enctype="multipart/form-data"`, preview kalau sudah ada, disimpan via `ImageService::attach()` koleksi `MarketplaceMaster::MASTER_IMAGE`). Simpan memanggil `setMasterStock`/`setMasterPrice` kalau field diisi (supaya `seeded_at` ke-set), lalu (saat Ubah) `pushMaster()`.
...
2. **Tambah Produk Baru** (`create`/`store`) — admin isi nama/SKU/harga/stok/tipe/foto master secara manual.
3. **Tambah ke Marketplace** — dari menu Atur master yang relevan, pilih listing yang belum tertaut → `tautkan` → stok/harga master mulai disinkron ke channel listing itu.
...
### Route / izin
`MarketplaceStockController` (nama kelas dipertahankan dari Fase 1 demi kestabilan URL `/marketplace-stock*`) — rute `index`/`create`/`store`/`edit`/`update`/`duplikat`/`channel`/`master.stok`/`master.harga`/`channel.stok`/`channel.harga`/`ikut-master`/`tautkan`/`push-all`/`resolve`/`master.hapus` (`DELETE`)/`master.bundle`. **Dihapus** (model auto sebelumnya): `siapkan`, `seed-tiktok`, `masterize-all`, `master.gabung`, push per-master (`push/{master}`), `master.foto` — foto sekarang ditangani langsung di form `create`/`update` (`enctype="multipart/form-data"`). View pakai `<select>`/input native, POST + `@csrf` biasa (tanpa literal `@json([...])`).
